...
namespace FoxHerding.Puzzle
{
    public class PuzzleGenerator : MonoBehaviour
    {
        ...
        private void GeneratePieces()
        {
            ...
            foreach (LevelData.PieceSpawnData levelPiece in _levelData.AvailablePieces)
            {
                foreach (PuzzlePiece packPiece in _currentPieceData.PackPieces)
                {
                    ...
                    if (levelPiece.FlipPiece)
                    {
                        Vector3 localScale = pieceTransform.localScale;
                        localScale = new Vector3(-localScale.x, localScale.y, localScale.z);
                        pieceTransform.localScale = localScale;
                    }
        ...
        {
            if (_gameHandler.CurrentLevel >= _gameHandler.AvailableLevels.Count)
            {
                _gameHandler.CurrentLevel = Mathf.Max(0, _gameHandler.CurrentLevel -=1);
